    ...
    @php
        ...
        if ($firstFileWithBarcode) {
            if ($firstFileWithBarcode->land) {
                $geoLocation = collect([
                    $firstFileWithBarcode->land?->district?->name ? '(حي : ' . $firstFileWithBarcode->land->district->name . ')' : null,
                    $firstFileWithBarcode->land?->area?->name ? '(مجاوره : ' . $firstFileWithBarcode->land->area->name . ')' : null,
                    $firstFileWithBarcode->land?->sector?->name ? '(قطاع : ' . $firstFileWithBarcode->land->sector->name . ')' : null,
    ...
    @php
        ...
        $clientFilesData = $client->files->filter(fn($f) => $f->barcode)->map(function($f) use ($client) {
            $fileGeo = $f->land ? collect([
                $f->land?->district?->name ? '(حي : ' . $f->land->district->name . ')' : null,
                $f->land?->area?->name ? '(مجاوره : ' . $f->land->area->name . ')' : null,
                $f->land?->sector?->name ? '(قطاع : ' . $f->land->sector->name . ')' : null,
